...
matrix_t* produitMatriciel(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; i < ne; i++)
    {
        ...
        for (int ax = 0, ay = y, bx = x, by = 0; ax < C->nc; ax++, by++) {
            v += A->v[CoordonneesAIndice(C, ax, ay)] * B->v[CoordonneesAIndice(C, bx, by)]; // <--
        }
...
matrix_t* produitMatriciel(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; i < nombreElements; i++) // <--
    {
        ...
        for (int xA = 0, yA = y, xB = x, yB = 0; xA < C->nc; xA++, yB++) {
            produit += A->v[CoordonneesAIndice(C, xA, yA)] * B->v[CoordonneesAIndice(C, xB, yB)]; // <--
        }
...
matrix_t* produitMatriciel(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; i < nombreElements; i++)
    {
        ...
        for (int xA = 0, yA = y, xB = x, yB = 0; xA < C->nombreColonnes; xA++, yB++) { // <--
            produit += A->elements[CoordonneesAIndice(C, xA, yA)] * B->elements[CoordonneesAIndice(C, xB, yB)]; // <--
        }
...
matrix_t* produitMatriciel(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; i < nombreElements; i++)
    {
        ...
        for (int xA = 0, yA = y, xB = x, yB = 0; xA < C->nombreColonnes; xA++, yB++) {
            produit += A->elements[i] * B->elements[i]; // i à la place de CoordonneesAIndice(C, x_, y_)
        }
...
matrix_t* produitMatriciel(matrix_t *A, matrix_t *B) {
    ...
    for (int i = 0; i < nombreElements; i++)
    {
        ...
        for (int xA = 0, yA = y, xB = x, yB = 0; xA < C->nombreColonnes; xA++, yB++) {
            int indice = CoordonneesAIndice(C, xA, yA);
...
A titre exclusivement personnel, mon code final ressemble à ça :
